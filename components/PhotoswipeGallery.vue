...
.gallery-grid {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax($minGridColumn,1fr));
...
@mixin maxColumnsMediaQuery($columns) {
  @media (max-width: calc((#{$minGridColumn} * #{$columns}) + (#{$gridGap} * (#{$columns} + 1)))) { @content; }
}

@include maxColumnsMediaQuery(3) {
  .grid-item {
    grid-column: span 1!important;
  }
}

@include maxColumnsMediaQuery(2) {
